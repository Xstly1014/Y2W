...
| ~~`static/`~~ | **已删除（第九轮）**。原 standalone agent 聊天页（端口 8000），现在用户唯一入口是 `ecommerce/static/shop/` | n/a |
| `ecommerce/static/shop/` | 用户唯一入口：web 商城 SPA + 内嵌客服 widget | 不引构建工具，纯原生 JS / Vue 3 CDN |
...
- **Web 平台（有状态）**：改了 `api/` 或 `mock_platform/` 或 `ecommerce/` 代码后，需要 Ctrl+C 停掉 `run_all.py` 然后重新 `python -m scripts.run_all`（uvicorn 未开 `--reload`）。改 `ecommerce/static/shop/*` 不需要重启，浏览器强刷即可（8000 端口的 standalone 页面已在第九轮删除）。

### 2.5 启动 Web 平台
```powershell
# 一键启动 mock_platform (8001) + api (8000) + ecommerce (8002) 三个服务
.venv\Scripts\python.exe -m scripts.run_all

# 等输出 "0719agent Commerce Platform is up" 后：
#   - 浏览器访问 http://127.0.0.1:8002/shop  ← 唯一用户入口（web 商城 + 内置客服 widget）
#   - Swagger 文档    http://127.0.0.1:8000/docs  (8000 是纯 API 端口，没有 HTML 页面)
...
73. **subprocess.PIPE 缓冲区满会反阻塞子进程，stream_output 必须与 wait_for 并发**
    - 现象：`scripts/run_all.py` 启动 api 子进程后，先顺序执行 `wait_for(8001) → wait_for(8000) → wait_for(8002)`，再启动 `stream_output` 线程消费 `proc.stdout`。结果 api 在 BGE warmup 时疯狂打 HuggingFace HEAD 请求日志（几十行），把 ~64KB 的 PIPE 缓冲区填满，api 的 `logger.info` 调用阻塞，warmup 卡住 84s，超过 wait_for 的 90s 超时，run_all 报 `[api] NOT ready after 90s`，但服务实际是好的（只是 stdout 卡了）。
    - 根因：`subprocess.Popen(stdout=PIPE)` 的 PIPE 有 OS 级缓冲区（Windows ~4KB，Linux ~64KB），父进程不读则子进程写阻塞。这是 Python subprocess 的经典坑。
    - 解决：在 `start()` 之后**立即**启动 `stream_output` 线程，让 stdout 被持续消费，与 `wait_for` 并发执行。同时把 `signal.signal` / `atexit.register` 也提前到 `stream_output` 之前，确保 wait_for 期间 Ctrl+C 仍能 cleanup。
    - 教训：**任何用 `subprocess.PIPE` + 后台线程消费 stdout 的代码，消费线程必须在 wait/health-check 之前启动**，否则子进程会被自己的日志卡死。或者用 `stdout=None`（继承父进程 stdout，直接写终端不经过 PIPE），但会失去 `[tag]` 前缀。

## 14. Kiki 模式实现（2026-07-21，第七轮）

对标腾讯云 Kiki 智能助手的交互模式：**动态步骤提示 → 中间回答 → 继续操作 → 最终回答 → 行动按钮**，不限于两段式，中间每一步的提示文字根据实际操作动态生成。

### 14.1 实现清单

| 模块 | 状态 | 说明 |
|------|------|------|
| 后端1 WebOpsSkill | ✅ 重写为 async | `skills/web_ops.py`：8 个工具（web_open_url/extract_text/list_links/click/fill/press_key/wait_for/screenshot），全部 `async def`，用 `playwright.async_api`，单 Browser + per-tenant BrowserContext，不阻塞事件循环 |
| 后端2 dynamic_progress | ✅ | `_friendly_for_tool` 静态映射覆盖所有 web_ops 工具 + `_refine_friendly` 后台 LLM 优化（LRU 缓存 512 条，opt-in via `WEB_DYNAMIC_PROGRESS=1`） |
| 后端3 action_card | ✅ | `_extract_action_cards` 解析 `[ACTION]json[/ACTION]` 块，SSE 发 `action_card` 事件 + `final` 事件携带 `action_cards` 列表 |
| 后端4 multi-turn | ✅ | `_StreamCtx.interim_buffer` 累积 LLM content chunks；chunk 有 tool_calls 时 flush buffer 为 `interim_answer` 事件；完整 AIMessage 有 content+tool_calls 时同样 flush |
| 后端5/6 WebOps 接入 | ✅ | `api/deps.py`：order_tools=14, knowledge_tools=16，两个子 agent 都注册了 8 个 web_ops 工具 |
| 后端7 SSE 扩展 | ✅ | `chat_stream` 发 `interim_answer` 事件（中间回答）+ `action_card` 事件（行动按钮） |
| 前端1 用户气泡 | ✅ | `.msg-bubble.user` min-width:80px, padding 加大 |
| 前端2 thinking-card | ✅ | 默认展开，无 collapsed 类 |
| 前端3 action_card | ✅ | `buildActionCard` 渲染底部按钮组，点击自动 sendMessage(prompt) |
| 前端4 interim_answer | ✅ | `handleSSEEvent` case 'interim_answer'：在 thinking-card 和最终 bubble 之间插入 `.msg-bubble.interim-answer` 气泡，支持多段 |

### 14.2 修改文件清单

- `skills/web_ops.py` — **全量重写**：sync_api → async_api，8 个工具改 `async def`，`_ensure_browser`/`_get_context`/`_get_page` 全改 async，移除 `threading.Lock` + `_lock()` contextmanager
- `api/routes/chat.py` — `_StreamCtx.interim_buffer` 字段；`_iter_message_events` chunk 分支累积 content 到 buffer、tool_call 到达时发 `interim_answer`；完整 AIMessage 分支 flush interim content；最终回答分支清空 buffer；`_friendly_for_tool` 新增 8 个 web_ops 工具映射；非流式 `chat()` 改 `async def` + `astream`（修 sync router 报错）
- `core/multi_agent.py` — `ROUTER_PROMPT` 新增网页操作请求路由到 knowledge 的规则；`ORDER_OPS_PROMPT` / `KNOWLEDGE_PROMPT` 的 Kiki 模式段落重写：明确指导 LLM 在多步操作时输出中间回答
- `static/index.html` — CSS `.msg-bubble.interim-answer` 样式；JS `handleSSEEvent` 新增 `case 'interim_answer'`
...
# 测试 Kiki 完整流程（web_ops + 中间回答 + 最终回答）
# 发送：帮我打开 https://example.com 看看内容，并列出页面上的所有链接
# 预期 SSE 事件流：
#   meta → step_start(router) → step_end → route(knowledge)
#   → interim_answer("好的，我来打开这个网页看看。")
#   → step_start(web_open_url) → step_end
#   → interim_answer("页面已打开。现在提取详细内容并列出所有链接。")
#   → step_start(web_extract_text) + step_start(web_list_links) → step_end x2
#   → step_start(llm_call) → step_end
#   → final（Markdown 表格 + 引用块）
#   → summary（~11s, 3 tools, 4 LLM, 5 steps）
```

### 14.4 新增坑位（继续编号）

74. **Playwright sync_api 在 async agent 中会冻结整个事件循环**
    - 现象：`web_ops.py` 最初用 `playwright.sync_api`，工具是同步 `def`。agent 通过 `astream` 异步执行，langgraph 对 sync tool 不会自动放到线程池（在 async 上下文中直接调用），`page.goto()` 阻塞了 FastAPI 事件循环。结果：web_open_url 被调用后整个 SSE 流冻结，只有 ping 心跳，90 秒超时无响应。
    - 根因：**sync Playwright 的所有 IO 操作（goto/click/fill）都是阻塞调用**，在 async 事件循环的线程中执行会卡死循环，所有并发请求（包括 SSE 心跳的发送）都被阻塞。
    - 解决：全量改用 `playwright.async_api`，8 个工具改 `async def`，`_ensure_browser` / `_get_context` / `_get_page` 全改 `async`。langgraph 的 `astream` 会 `await` async tool，事件循环在等待浏览器 IO 时可以处理其他请求。
    - 教训：**在 FastAPI + langgraph async 环境中，所有可能阻塞的工具（浏览器 / 文件 IO / 网络）必须用 async API 实现**。sync tool 在 async 上下文中不是自动线程池执行，而是直接阻塞事件循环。

75. **Router 把网页操作请求误路由到 escalation（人工客服）**
    - 现象：用户说"帮我打开 example.com 看看内容"，Router 判断"请求内容与跨境电商客服无关，需转人工处理"，路由到 escalation。web_ops 工具根本没被触发。
    - 根因：`ROUTER_PROMPT` 的分类规则只列了订单/知识/升级三类，没有明确说"打开网页/查看网站"应该路由到 knowledge（知识库专员有 web_ops 工具）。LLM 把网页操作当成了无关请求。
    - 解决：在 `ROUTER_PROMPT` 的 knowledge 规则中明确加入"**包括买家要求打开网页/查看网站/帮我操作网页/帮我领券等需要用浏览器的请求**"，并在 escalation 规则中加"仅仅是打开网页/查看网站本身不构成升级理由"。
    - 教训：**Router prompt 的分类规则要覆盖所有子 agent 的能力边界**。新增工具（web_ops）后必须同步更新 Router 的路由规则，否则工具注册了但永远不会被触发。

76. **非流式 chat 接口调用 async router_node 报 "No synchronous function provided"**
    - 现象：`/api/chat`（非流式）返回 `[agent error] No synchronous function provided to "router"`。
    - 根因：`core/multi_agent.py` 的 `router_node` 是 `async def`（P0-2 优化要求，避免阻塞事件循环），但 `api/routes/chat.py` 的 `chat()` 是同步 `def`，调用 `agent.stream()`（同步迭代器）。langgraph 的同步 stream 无法执行 async node。
    - 解决：`chat()` 改 `async def`，`agent.stream()` 改 `async for event in agent.astream()`。`agent.get_state` 保持同步（无 LLM 调用，短时可接受）。
    - 教训：**graph 中有任意一个 async node，整个 graph 必须用 async API（astream/ainvoke）驱动**，不能混用 sync stream + async node。

77. **Mock LLM 替代上游：401 token 失效导致演示完全瘫痪**
    - 现象：上游 OpenAI-compatible 代理（NewAPI/OneAPI）的 token 过期/被吊销后，所有 ReAct 子 agent 的 LLM 调用都 401，前端只看到一句 `[agent error] Error code: 401`。演示完全跑不通。
    - 根因：`build_llm()` 只构造 `ChatOpenAI`，没有离线 fallback。
    - 解决：新增 `core/mock_llm.py`（`MockChatModel`）——基于关键词（订单/物流/退款/网页/政策/升级）驱动 ReAct 决策，生成符合 langgraph 期望的 `AIMessage` / `AIMessageChunk` / `ToolMessage` shape。`config/settings.py` 新增 `llm_mock: bool = False`，`.env` 设 `LLM_MOCK=1` 启用。`build_llm()` 根据该开关返回 mock 或真实 LLM。
    - 教训：**任何依赖外部 API 的核心链路都要有离线 fallback**。LLM 调用失败时的兜底是"假装在思考 + 按关键词出固定答案"，比直接挂掉强一万倍——尤其 demo 场景。

78. **Mock LLM 工具返回非 JSON 文本时 final_answer 显示硬编码默认值**
    - 现象：mock LLM 调用 `query_order` 后，用户看到"已发货 / 顺丰速运 / TRACK-1001-US"——即使平台实际返回的 `order#1001 status=refunded tracking= N/A`。`create_refund` 失败时仍然显示"已处理"。
    - 根因：mock_llm 的 `_order_final_answer` 只 try `json.loads(tool_payload)`，解析失败就 fallback 到硬编码"1001/shipped/TRACK-1001-US"，丢失了工具返回的真实状态。`query_logistics` 的 `latest_events: ...` 文本格式、`create_refund` 的 `refund rejected: ...` 文本格式都没解析。
    - 解决：实现三个专用 parser：
      - `_parse_order_text`：正则提取 `order#X` / `status=X` / `total=$X` / `tracking=X` / `refundable=Bool` / `items=[...]`；当 `tracking=N/A` 时不设 `tracking_no` 字段
      - `_parse_logistics_text`：提取 `tracking=X` + `latest_events` 后的 `(time, status, location)` 三元组，转成 timeline list
      - `_parse_refund_text`：识别 `rejected` 关键词 + 提取 `reason_message`
      - `_order_final_answer` 根据解析结果动态渲染：status 通过 `_STATUS_CN` 映射成中文（refunded→已退款），tracking 为空时显示 `—`，can_refund=False 时不渲染"申请退款"按钮，create_refund 拒绝时返回 `● 未通过` 而非 `● 已处理`。
    - 教训：**mock 实现也得忠实于数据**。硬编码默认值在数据不匹配时会误导用户和后续 agent 决策。工具返回的非 JSON 文本必须配专用解析函数，不能简单 fallback。`tracking=N/A` 这种"显式空值"要和"字段缺失"区分开——前者用户应该看到 `—`，后者走另一条分支。

79. **同一个项目里"客服页面"出现两个入口会误导用户**
    - 现象：web 商城上线后，`http://127.0.0.1:8000/`（agent 独立页）和 `http://127.0.0.1:8002/shop`（商城内置客服 widget）都能发起客服对话，但两边的 UI 样式、状态管理、订单上下文**完全独立**——同一个用户切到另一个入口会看到"另一个机器人"，因为 thread_id / session 不互通，体验割裂。
    - 根因：早期 agent 后端自带了一个 standalone HTML 页面做 demo，后期接入 web 商城后忘了删旧入口；用户看到两个 URL 都能用，理所当然会随便点。
    - 解决：**前端入口唯一性原则**——一个项目只允许一个用户可见的入口。Agent 后端 (`api/server.py`) 删除所有 `@app.get("/...")` 的 FileResponse 路由 + `StaticFiles` mount，只保留 `/api/*` JSON API；`static/` 目录整个删掉；`scripts/run_all.py` 启动 banner 显式标注 user-facing URL。
    - 教训：**有"两条路"通向同一个功能时，删一条**。多入口的维护成本（双份 UI、双份测试、双份 bug 报告）远超它的便利性。如果想保留 standalone demo，把它放另一个端口（比如 8003）并明确标注"[DEV ONLY]"，不要和生产入口混在同一个服务里。

### 14.5 Kiki pacing v2：把 LLM 延迟"匀"到前面的不耗时步骤（2026-07-21）

第七轮用户反馈：步骤瞬间完成 + 慢 LLM 突兀等待 → 要求"把耗时的 llm 调用过程的延迟匀到前面的不耗时操作上，让每一步看起来都在思考、每一步延迟都不高"。

**v1 实现的痛点**：`_MIN_STEP_DURATION_S=0.6` 是固定下限。快步骤（router/agent_think）hold 到 0.6s 后立即翻 done；接下来的慢 LLM（5s）一次性吞掉剩余时间，用户体感是"前几个一闪而过 → 突然卡 5s → 又一闪而过"，不像在"做事"。

**v2 实现的核心思想**：把 per-step 固定下限改成 **基于流总预算的动态 hold**。

```python
# 关键 4 个 env-tunable 常量
_MIN_STEP_DURATION_S = 1.0   # 单步 hold 上限
_TARGET_TOTAL_S     = 10.0   # 整流总时间预算
_TYPICAL_STEPS      = 6      # 估算总步数（用于分摊）
_MIN_HOLD_FLOOR_S   = 0.4    # 单步 hold 下限（避免一帧闪过）

# 在 _StreamCtx 维护 stream_started_at + steps_completed
# 每个 step_end 到来时：
stream_elapsed     = time.time() - ctx.stream_started_at
remaining_budget   = max(0, _TARGET_TOTAL_S - stream_elapsed)
steps_left         = max(1, _TYPICAL_STEPS - ctx.steps_completed)
per_step_budget    = remaining_budget / steps_left
target_hold        = min(_MIN_STEP_DURATION_S, per_step_budget)
target_hold        = max(_MIN_HOLD_FLOOR_S, target_hold)
sleep_for          = max(0, target_hold - actual_elapsed)
```

**效果**（单测 `probe_pacing.py` 输出）：
```
场景：快-router + 快-think + 快-tool + 慢-LLM(5s) + 快-final + 快-tail
  step#1 actual=0.05s  hold=0.95s  displayed=1.00s
  step#2 actual=0.05s  hold=0.95s  displayed=1.00s
  step#3 actual=0.05s  hold=0.95s  displayed=1.00s
  step#4 actual=5.00s  hold=0.00s  displayed=5.00s   ← LLM 不被压缩
  step#5 actual=0.05s  hold=0.93s  displayed=0.98s
  step#6 actual=0.05s  hold=0.92s  displayed=0.97s
  TOTAL displayed = 9.95s  (≈ budget 10s)
```

- 慢 LLM 保持真实耗时 5s，不被压缩
- 前后快步骤 hold 到 ~1.0s（MIN 上限），不再瞬间闪过
- 总时间被 budget 控制，不会无限延长
- 一旦 budget 用完，hold 退到 FLOOR (0.4s) 避免步骤消失

**修改文件**：
- `api/routes/chat.py` — 4 个新常量；`_StreamCtx` 加 `stream_started_at` + `steps_completed`；`event_gen` 的 messages 分支 step_end 处理改用新公式；`event_gen` 的 router 分支同步更新并加 `ctx.steps_completed += 1`

**为什么是"看起来"匀而不是真匀**：流式场景下，步骤 1-3 已经 yield 到前端，前端已经显示了 1.0s 的"正在判断"等提示——这就是"匀"的视觉效果。我们不可能让快步骤真的等 5 秒再 yield（那样总时间变 15s+），而是通过 **per-step hold = f(remaining_budget)** 让快步骤在 budget 充裕时多 hold 一些，budget 紧张时少 hold，让每个 step 在屏幕上"驻留"的时间相对均匀。

**SSE 验证**（curl probe_stream.py）：
```
[  0.09s] START step#1 type=agent_think msg=正在判断
[  1.10s] END   step#1 latency_ms=1006.8                  ← hold 到 1.0s
[  1.10s] ROUTE → 知识库专员 (knowledge)
```
（上游 LLM token 失效导致后续步骤 401，但 router 步骤已确认 hold 1006.8ms ≈ 1.0s）

### 14.6 第八轮：LLM Mock + 前端去重 + 行动按钮导航（2026-07-21）

第七轮上线后用户截图反馈三个新问题：(1) 用户提问气泡太短小，文字被截断；(2) LLM 中间回答"正在查询订单 1001 状态..."与 step_start 的 `friendly_message` 重复显示；(3) 「查看物流」「申请退款」按钮点击无反应。

**问题 1 修复（前端）**：`ecommerce/static/shop/styles.css` 调整 `.cs-message-self-bubble`：
- `max-width: 80% → 92%`，允许气泡占满更多宽度
- `padding: 8px 12px → 10px 14px`，内部留白更大
- `font-size: 13px → 14px`，可读性更好
- `.cs-message-self` 的 `max-width` 从 88% → 96%，让气泡可以更接近右边

**问题 2 修复（前后端双重去重）**：
- 前端 `components.js` 的 `interim_answer` handler 在渲染前对比 `aiMsg.reasoning` 最后一步的 `friendly_message`，正则去掉末尾标点（`。…，！？.,!?`）后 normalize 比对，相同则 skip；不同时 push 到 `aiMsg.interimAnswers`
- 后端 `api/routes/chat.py` 的 `_iter_message_events` 在 tool_call chunk 到达时 flush `interim_buffer` 前，对比 `interim_buffer` 与下一个 step 的 `_friendly_progress`；相同则清空 buffer 不发 `interim_answer`；完整 AIMessage 路径同样的去重

**问题 3 修复（Action Card 点击处理）**：`components.js` 新增 `onActionCard(card)` 函数：
- 优先级：`card.href` → 用 `router.push` 导航
- 否则按 `card.label` 查表（`查看物流` / `申请退款` / `查看我的订单` 都导航到 `/orders`）
- 否则用 `card.prompt` 注入 input 并触发 `send()`
- 模板中 `@click="onActionCard(card)"` 替代之前的 `sendMessage(card.prompt)`
- 同时 `sending` 状态时拦截点击，避免重复请求

**修改文件**：
- `core/mock_llm.py` — 新增（mock 离线 LLM）
- `core/llm.py` — `build_llm()` 根据 `settings.llm_mock` 返回 mock 或真实 LLM
- `config/settings.py` — 新增 `llm_mock: bool = False` 字段
- `api/routes/chat.py` — 双重去重逻辑
- `ecommerce/static/shop/components.js` — `onActionCard` + interim_answer 去重
- `ecommerce/static/shop/styles.css` — 用户气泡样式
- `probe_1003.py` / `probe_logistics.py` / `probe_refund.py` / `probe_mock.py` / `probe_raw.py` — 探针脚本

**关键决策**：
- 真实 LLM 在线时仍走 ChatOpenAI，mock 仅作为离线 fallback，不影响生产路径
...
### 14.7 第九轮：删除 8000 端口 standalone agent 页面（2026-07-21）

第八轮修复后用户反馈：客服入口**只**应该是 web 商城（端口 8002 → `/shop`），端口 8000 上挂的 standalone agent 聊天页 `static/index.html` + 运营后台 `static/admin.html` 是历史遗留，**直接删掉**。本轮把"页面"和"后端"彻底拆开——前端 UI 完全由电商服务负责，8000 端口只暴露 `/api/*` JSON API 给电商服务的客服 widget 调用。

**改动**：
- 删除整个 `static/` 目录（含 `index.html` / `admin.html` / `avatars/ai_avatar.jpg`）
- `api/server.py`：删除 `STATIC_DIR` mount、`/static` + `/avatars` StaticFiles、`@app.get("/")` + `@app.get("/admin")` 两个 FileResponse 路由；移除 `pathlib` / `fastapi.responses.FileResponse` / `fastapi.staticfiles.StaticFiles` 导入
- `api/__init__.py`：更新模块 docstring，明确"API-only"
- `scripts/run_all.py`：启动 banner 改为 `Agent API: http://127.0.0.1:8000/api/health (no HTML page)`，把 user-facing 入口标注为 `http://127.0.0.1:8002/shop`

**保留**：8000 端口的 `/api/chat/stream` / `/api/health` / `/api/kb` / `/api/feedback` / `/api/traces` / `/api/flywheel` / `/api/dashboard` / `/api/metrics` 全部不动——电商服务的 `routes/customer_service.py` 通过 `ECOMMERCE_AGENT_API_URL=http://127.0.0.1:8000` 代理过来。

**验证**（probe_static_removed.py，提交后删除）：
```
GET /            -> 404    ✓ static page gone
GET /admin       -> 404    ✓
GET /api/health  -> 200    ✓ API intact
POST /api/chat/stream {"message": "hi"} -> 200  ✓ end-to-end stream still works
133 tests pass
```

**为什么不在 `api/server.py` 加 `redirect "/" -> "http://...:8002/shop"`**：
- redirect 会让首次访问 8000 的用户看到跳转，体验差（"我的页面怎么没了？"）
- 8000 是**后端服务**——它没有"页面"可言；只允许 API 调用方（电商服务）主动连接
- 用户遇到 8000 直接 404，反而能立刻意识到"这是 API 端口，不是用户入口"
